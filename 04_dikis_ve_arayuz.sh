#!/usr/bin/env bash
#
# Faz 1 / Adim 7 — Dikis dagitici
# + apps/web — Vercel'e yayinlanabilir arayuz
#
# Kullanim:
#   chmod +x 04_dikis_ve_arayuz.sh
#   ./04_dikis_ve_arayuz.sh
#
...
echo "==> Dizinler"
mkdir -p packages/geometry/src/stitch packages/patterns/src apps/web/src

echo "==> packages/geometry/src/stitch/corners.ts"
cat > packages/geometry/src/stitch/corners.ts << 'ODK_EOF_0'
...
cat > packages/geometry/src/stitch/distribute.ts << 'ODK_EOF_1'
import type { Mm } from "../units.js";
...
cat > packages/geometry/src/stitch/stitch.test.ts << 'ODK_EOF_2'
...
echo "==> packages/geometry/src/index.ts"
cat > packages/geometry/src/index.ts << 'ODK_EOF_3'
/**
...
cat > packages/patterns/src/cardholder.ts << 'ODK_EOF_4'
...
cat > packages/patterns/src/cardholder.test.ts << 'ODK_EOF_5'
import { describe, it, expect } from "vitest";
...
cat > packages/patterns/src/index.ts << 'ODK_EOF_6'
/**
...
cat > apps/web/package.json << 'ODK_EOF_7'
{
  "name": "@odk/web",
  "version": "0.0.1",
  "private": true,
...
cat > apps/web/tsconfig.json << 'ODK_EOF_8'
{
...
echo "==> apps/web/vite.config.ts"
cat > apps/web/vite.config.ts << 'ODK_EOF_9'
...
cat > apps/web/src/main.tsx << 'ODK_EOF_11'
...
cat > apps/web/src/engine.ts << 'ODK_EOF_12'
/**
 * Motor köprüsü.
 *
...
echo "==> apps/web/src/PieceView.tsx"
cat > apps/web/src/PieceView.tsx << 'ODK_EOF_13'
...
cat > apps/web/src/App.tsx << 'ODK_EOF_14'
...
cat > apps/web/src/styles.css << 'ODK_EOF_15'
/*
  TASARIM NOTU
  ────────────────────────────────────────────────────────────────────
...
echo "==> vercel.json"
cat > vercel.json << 'ODK_EOF_16'
{
  "$schema": "https://openapi.vercel.sh/vercel.json",
  "buildCommand": "pnpm --filter @odk/web build",
...
Git:
  ...
  git add -A
  git commit -m "Faz 1 Adim 7: dikis dagitici + web arayuzu

- roundCorners: kose yuvarlatma ayri ve bilincli adim (Adim 5 bulgusu)
- distributeStitches: koseler capa, segment basina round(L/pitch),
  gercek adim L/n
- selectPitch: minimax — tek parcada tek iron kullanilabildigi icin
  adim global secilir, en kotu segmentteki sapma minimize edilir
- generateCardHolder: parametre -> kesit -> parca hatlari -> dikis plani
- apps/web: React + Vite, SVG onizleme, kesim mati izgarasi 1:1 mm
- vercel.json: monorepo derlemesi
- 214 test geciyor"

  git push

Vercel'e yayinlamak icin:

  npm i -g vercel
  vercel login
  vercel --prod

  # Vercel sorarsa: root directory = repo koku (apps/web DEGIL).
  # vercel.json zaten build komutunu ve cikti dizinini tanimliyor.
  #
  # Alternatif: vercel.com uzerinden GitHub reposunu import et.
  # Framework Preset: Other. Diger ayarlar vercel.json'dan gelir.

Sonraki adim: Faz 2 — PDF/baski katmani (kalibrasyon karesi + A4 tiling).
ODK_DONE
